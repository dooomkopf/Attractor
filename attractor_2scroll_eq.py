...
STEP      = 40     # Punkte pro Frame (höher = schneller)
...
def _wang_jac(x, y, z):
    """Analytischer Jacobian des Wang-2-Scroll-Systems an (x, y, z)."""
    return np.array([
        [A,      -A - z,  -y],
        [z,      -B,       x],
        [D + y,   x,      -C],
...
def compute_ftle_wang(traj, dt):
    """FTLE λ1 via QR-Gram-Schmidt. Gibt λ1 in s⁻¹ zurück."""
    Q = np.eye(3)
    log_sum = 0.0
    for pt in traj:
        J = _wang_jac(*pt)
        Q, R = np.linalg.qr((np.eye(3) + dt * J) @ Q)
        log_sum += np.log(max(abs(R[0, 0]), 1e-300))
    return log_sum / (len(traj) * dt)
...
def main():
    ...
    dt_ref = t_ref / n_ref
    ftle_wang = compute_ftle_wang(traj_pool, dt_ref)
    print(f"FTLE λ1 (Wang) = {ftle_wang:+.4f} s⁻¹")
    ...
    with plt.rc_context({'text.usetex': False}):
        plt.suptitle('Wang 2-scroll Attraktor', color='#CCCCCC', fontsize=13,
                     y=0.99, fontname='Comfortaa', fontweight='bold')
    ...
    def update(_frame):
        ...
        #     f'k1={k1_ref:.3f} k2={k2_ref:.3f}  FTLE λ1={ftle_wang:+.3f} s⁻¹  '
        #     f'cycle={state["traj_idx"]+1}/{N_TRAJS}  t={t_now:.1f}')

        ax.azim = (ax.azim + 0.3) % 360
