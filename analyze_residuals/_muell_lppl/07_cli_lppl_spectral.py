#!/usr/bin/env python3
"""CLI: LPPL spectral analysis + master/slave selection at equilibrium."""
...
def main():
    ap = argparse.ArgumentParser(description='LPPL spectral analysis + master/slave selection')
    for k, v in DEFAULT_PARAMS.items():
        ap.add_argument(f'--{k}', type=float, default=v)
    ap.add_argument('--eq', type=str, default='E2', help='equilibrium label (default: E2, stable focus)')
    args = ap.parse_args()
    ...
    print("=" * 84)
    print(f"LPPL SPECTRAL ANALYSIS AT {args.eq}: ({eq_point[0]:+.6f}, {eq_point[1]:+.6f}, {eq_point[2]:+.6f})")
    print("=" * 84)
