...
def plot_eigenvalues(ax, ana, title, legend_loc='upper right',
    ...
    """Complex-plane plot of M's eigenvalues with all oscillatory pairs labelled."""
    eigs = ana['eigvals']
    if eigs is None:
...
def main():
    ...
    with plt.rc_context({'text.usetex': False}):
        plt.suptitle(f'BTC residuals: harmonics test on log-clock SSM  '
    ...
    if args.save is None:
        args.save = os.path.join(
            HERE,
            f'13_2_lambda2_visualize_dim{args.ssm_dim}_poly{args.poly}.png',
        )
