...
Fenster 1: Residuen (oben) + PCA 3D Attraktor (unten) + Slider (±30d Fenster).
...
START_IDX         = 1164
TAU               = 60
YEARS             = 8              # Fensterbreite → M automatisch
M                 = round(YEARS * 365 / TAU) + 1
...
CYCLE_TOPS        = [
    (datetime(2013, 12, 4), "'13"),
    (datetime(2017, 12, 16), "'17"),
    (datetime(2021, 11, 9), "'21"),
    (datetime(2025, 10, 7), "'25"),
]
# ──────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    def _cycle_color(day_idx):
        for _s, _e, _c, _ in _periods:
            if _s <= day_idx < _e:
                return _c
        return 'orange'

    colors_all = np.array([_cycle_color(d) for d in days_all])
    ...
    colors_vec = np.array([_cycle_color(d) for d in days_vecs])
    ...
    lc1  = LineCollection(segs, colors=colors_all[:-1], linewidth=1.0, alpha=0.9)
    ...
    if len(used_peak_days):
        ax1.scatter(used_peak_days, used_peak_vals,
                    c=[_cycle_color(d) for d in used_peak_days], s=50,
                    zorder=5, alpha=0.8)
    ...
    ax1.scatter(days_all[-1], residuals_all[-1], color='white', s=20,
                zorder=10, edgecolors='black', linewidth=0.8, label='Latest')
    for i, hday in enumerate(halving_days):
        hidx = np.argmin(np.abs(days_all - hday))
        ax1.scatter(days_all[hidx], residuals_all[hidx], color='grey', s=60,
                    zorder=10, label='Halving' if i == 0 else '_')
        ax1.annotate(str(i + 1), (days_all[hidx], residuals_all[hidx]),
                     textcoords='offset points', xytext=(5, 18 if i == 2 else 5),
                     color='grey', fontsize=9, fontweight='bold')

    # ── ETF-Marker 11. Jan 2024 ───────────────────────────────────────────────
    _etf_date    = datetime(2024, 1, 11)
    _etf_idx     = np.argmin(np.array([abs((d - _etf_date).days) for d in dates_all]))
    _etf_vec_idx = np.argmin(np.abs(days_vecs - days_all[_etf_idx]))
    ax1.scatter(days_all[_etf_idx], residuals_all[_etf_idx], color='red', s=60, zorder=11)
    ax1.annotate('ETFs', (days_all[_etf_idx], residuals_all[_etf_idx]),
                 textcoords='offset points', xytext=(6, -14),
                 color='red', fontsize=9, fontweight='bold')

    _cy_patches = [Patch(color=_c, label=_n) for _, _, _c, _n in _periods if _n]
    ...
    ax1.legend(handles=_cy_patches, loc='upper left', framealpha=0.9, fontsize=8)

    # ── ax2: PCA 3D ───────────────────────────────────────────────────────────
    segs3d = make_segments3d(pc_s[:, 0], pc_s[:, 1], pc_s[:, 2])
    lc2    = Line3DCollection(segs3d, colors=colors_vec[:-1], linewidth=2.5, alpha=0.9)
    ...
    for pd_, lbl in zip(used_peak_days, peak_year_labels):
        ...
        if np.abs(days_vecs[idx_] - pd_) < 200:
            ax2.scatter(pc_s[idx_, 0], pc_s[idx_, 1], pc_s[idx_, 2],
                        color=_cycle_color(pd_), s=60, zorder=10)
    ...
    ax2.scatter(pc_s[_etf_vec_idx, 0], pc_s[_etf_vec_idx, 1], pc_s[_etf_vec_idx, 2],
                color='red', s=60, zorder=11)
    ax2.text(pc_s[_etf_vec_idx, 0], pc_s[_etf_vec_idx, 1], pc_s[_etf_vec_idx, 2],
             ' ETFs', color='red', fontsize=9, fontweight='bold')
    ax2.scatter(*[pc_s[-1, i] for i in range(3)],
                color='white', s=25, zorder=10, label='Latest Data')
    ...
    ax2.set_title(f'PCA 3D Attractor  ·  τ={TAU}d · m={M} · W={W}d  (cum. {cum3:.1f}%)',
                  color='#E0E0E0', fontsize=10)
    ax2.tick_params(colors='#CCCCCC')
    ax2.legend(handles=_cy_patches, loc='upper left', fontsize=8, framealpha=0.7)


    # ── Slider ────────────────────────────────────────────────────────────────
    slider = Slider(ax_sl, 'Day', float(days_vecs[0]), float(days_vecs[-1]),
    ...
    ax3a.scatter(theta, r, c=colors_vec, s=4, alpha=0.85, linewidths=0)
    ax3a.set_ylim(0, r.max() * 1.1)
    ax3a.scatter(theta[-1], r[-1], color='white', s=25, zorder=10, label='Latest Data')
    for pd_, lbl in zip(used_peak_days, peak_year_labels):
        idx_ = np.argmin(np.abs(days_vecs - pd_))
        if np.abs(days_vecs[idx_] - pd_) < 200:
            ax3a.scatter(theta[idx_], r[idx_], color=_cycle_color(pd_), s=60, zorder=10)
            ax3a.annotate(lbl, (theta[idx_], r[idx_]), color='white',
                          fontsize=9, fontweight='bold')
    ax3a.scatter(theta[_etf_vec_idx], r[_etf_vec_idx], color='red', s=60, zorder=11)
    ax3a.annotate('ETFs', (theta[_etf_vec_idx], r[_etf_vec_idx]),
                  color='red', fontsize=9, fontweight='bold')
    ...
    lc3b = ax3b.scatter(theta, pc[:, 2], c=colors_vec,
                        s=4, alpha=0.7, linewidths=0)
    ax3b.scatter(theta[-1], pc[-1, 2], color='white', s=25, zorder=10, label='Latest Data')
    for pd_, lbl in zip(used_peak_days, peak_year_labels):
        idx_ = np.argmin(np.abs(days_vecs - pd_))
        if np.abs(days_vecs[idx_] - pd_) < 200:
            ax3b.scatter(theta[idx_], pc[idx_, 2], color=_cycle_color(pd_), s=60, zorder=10)
            ax3b.annotate(lbl, (theta[idx_], pc[idx_, 2]),
                          textcoords='offset points', xytext=(6, 4),
                          color='white', fontsize=9, fontweight='bold')
    ax3b.scatter(theta[_etf_vec_idx], pc[_etf_vec_idx, 2], color='red', s=60, zorder=11)
    ax3b.annotate('ETFs', (theta[_etf_vec_idx], pc[_etf_vec_idx, 2]),
                  textcoords='offset points', xytext=(6, 4),
                  color='red', fontsize=9, fontweight='bold')
    ax3b.set_xlabel('θ = atan2(PC2, PC1)  [Cycle Phase]', color='#E0E0E0')
    ...
    for i, hday in enumerate(halving_days):
        ...
        if np.abs(days_vecs[hidx] - hday) < 200:
            ...
            ax3a.scatter(theta[hidx], r[hidx], color='grey', s=60,
                         zorder=10, edgecolors='white', linewidths=1.0, label=lbl_h)
            ax3a.annotate(str(i + 1), (theta[hidx], r[hidx]),
                          color='white', fontsize=9, fontweight='bold')
            ax3b.scatter(theta[hidx], pc[hidx, 2], color='grey', s=60,
                         zorder=10, edgecolors='white', linewidths=1.0, label=lbl_h)
            ax3b.annotate(str(i + 1), (theta[hidx], pc[hidx, 2]),
                          textcoords='offset points', xytext=(5, 5),
                          color='white', fontsize=9, fontweight='bold')

    _now_patch  = Line2D([], [], color='white', marker='o', markersize=6, linestyle='None', label='Latest Data')
    _halv_patch = Line2D([], [], color='grey',  marker='o', markersize=6, linestyle='None', label='Halving')
    _etf_patch  = Line2D([], [], color='red',   marker='o', markersize=6, linestyle='None', label='ETFs')
    ax3a.legend(handles=_cy_patches + [_now_patch, _halv_patch, _etf_patch], loc='lower right', fontsize=8)
    ax3b.legend(handles=_cy_patches + [_now_patch, _halv_patch, _etf_patch], fontsize=8)
    fig3.tight_layout()
    ...
    ax4a.scatter(days_vecs, theta_uw, c=colors_vec, s=4, alpha=0.85, linewidths=0)
    _halving_on(ax4a, halving_days)
    ax4a.scatter(days_vecs[_etf_vec_idx], theta_uw[_etf_vec_idx], color='red', s=60, zorder=11)
    ax4a.annotate('ETFs', (days_vecs[_etf_vec_idx], theta_uw[_etf_vec_idx]),
    ...
    ax4a.legend(handles=[_halv_patch, _etf_patch], fontsize=8)

    ax4b.set_facecolor('#1A1A1A')
    log_days_vecs = np.log(days_vecs)
    ax4b.scatter(log_days_vecs, theta_uw, c=colors_vec, s=4, alpha=0.85, linewidths=0)
    _halving_on(ax4b, np.log(halving_days))
    ax4b.scatter(np.log(days_vecs[_etf_vec_idx]), theta_uw[_etf_vec_idx], color='red', s=60, zorder=11)
    ax4b.annotate('ETFs', (np.log(days_vecs[_etf_vec_idx]), theta_uw[_etf_vec_idx]),
                  textcoords='offset points', xytext=(-40, -14),
                  color='red', fontsize=9, fontweight='bold')
    ...
    ax4b.legend(handles=[_halv_patch, _etf_patch], fontsize=8)
    fig4.tight_layout()
