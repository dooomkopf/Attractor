...
def main():
    # ── CLI-Argumente ─────────────────────────────────────────────────────────
    ap = argparse.ArgumentParser(description='BTC Attraktor-Analyse')
    ap.add_argument('--W',     type=int,   default=None, help='Fensterbreite in Tagen')
    ap.add_argument('--years', type=float, default=None, help='Fensterbreite in Jahren')
    ap.add_argument('--M',     type=int,   default=35,   help='Embedding dimensions (default 35)')
    args = ap.parse_args()
    if args.years is not None or args.W is not None:
        W_in = round(args.years * 365) if args.years is not None else args.W
        M    = args.M
        TAU  = max(1, round(W_in / (M - 1)))
    else:
        M    = args.M
        TAU  = max(1, round(round(3.77 * 365) / (M - 1)))
    ...
    # Ensure CCW rotation (theta_uw increasing)
    if theta_uw[-1] < theta_uw[0]:
        theta    = -theta
        theta_uw = -theta_uw

    # Anchor: Halving 2 (2016) = Phase 0
    # H1 (2012) liegt zu nah am Embedding-Start → H2 ist erste stabile Referenz.
    # ACHTUNG: theta_uw wird hier mit theta[hidx_h2] (gewrappter Winkel) verschoben,
    # nicht mit theta_uw[hidx_h2]. H2 landet daher im unwrapped-Plot bei ~2π, nicht 0.
    # (Identisches Verhalten in attractor_BT.py ist dort korrigiert.)
    hidx_h2 = np.argmin(np.abs(days_vecs - halving_days[1]))
    if np.abs(days_vecs[hidx_h2] - halving_days[1]) < 200:
        _shift   = float(theta[hidx_h2])
    ...
    for ci in range(1, len(bounds) - 2):  # nur vollständige Zyklen (Mitte)
        ...
        dur_days = days_vecs[i2] - days_vecs[i1]
        dur_yr = dur_days / 365.25
        cycle_durs.append(dur_days)
    ...
    if len(cross_idx) >= 1 and len(cycle_durs) >= 1:
        ...
        last_cross_day = days_vecs[cross_idx[-1]]
        next_cross_day = last_cross_day + med_dur
        cycle_start_day = last_cross_day
        current_day     = days_vecs[-1]
        pct = (current_day - cycle_start_day) / med_dur * 100
        pct = max(0.0, min(pct, 100.0))
        print(f"Aktueller Zyklus: {pct:.1f}%  "
              f"(Start: Tag {int(cycle_start_day)}, "
              f"Median-Länge: {med_dur/365.25:.2f}y, "
              f"nächste Grenze: Tag {int(next_cross_day)} "
              f"≈ {next_cross_day/365.25:.1f}y)")
        ax1.axvline(next_cross_day, color='#FFDD44', linewidth=0.8,
                    alpha=0.6, linestyle=':',
                    label=f'Next boundary ({med_dur/365.25:.1f}y median)')
    else:
        next_cross_day = d_max

    ax1.set_xlim(0, max(d_max, next_cross_day) * 1.05)
    ax1.set_ylabel('rel. Value', color='#E0E0E0')
    ax1.set_title('BTC USD Residuals', color='#E0E0E0', fontsize=11, usetex=False)
    ...
    ax2.set_title(f'PCA 3D Attraktor  -  tau={TAU}d - m={M} - W={W}d  (kum. {cum3:.1f}%)',
                  color='#E0E0E0', fontsize=10, usetex=False)
    ...
    ax3a.set_rticks([])
    ax3a.set_title('Phase vs Intensity', color='#E0E0E0', fontsize=10, pad=15, usetex=False)
    ...
    ax3b.set_title('Phase', color='#E0E0E0', fontsize=10, usetex=False)
    ...
    cb3.set_label('Year', color='#CCCCCC')
    cb3_ticks = np.linspace(t_norm_vec[0], t_norm_vec[-1], 6)
    cb3_labels = [dates_all[np.argmin(np.abs(days_all - (t * (d_max - d_min) + d_min)))].strftime('%Y')
                  for t in cb3_ticks]
    ...
    for ci in cross_idx:
        ax4a.axvline(days_vecs[ci], color='#FFDD44', linewidth=0.8, alpha=0.6, linestyle=':')
    ax4a.axvline(next_cross_day, color='#FFDD44', linewidth=0.8, alpha=0.6, linestyle=':')
    ax4a.set_xlabel('Day index  (linear)', color='#E0E0E0')
    ax4a.set_ylabel(r'$\theta$ unwrapped  [rad]', color='#E0E0E0')
    ...
    for ci in cross_idx:
        ax4b.axvline(np.log(days_vecs[ci]), color='#FFDD44', linewidth=0.8, alpha=0.6, linestyle=':')
    ax4b.axvline(np.log(next_cross_day), color='#FFDD44', linewidth=0.8, alpha=0.6, linestyle=':')
    ax4b.set_xlabel('log(Day index)', color='#E0E0E0')
    ax4b.set_ylabel(r'$\theta$ unwrapped  [rad]', color='#E0E0E0')
