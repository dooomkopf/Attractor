#!/usr/bin/env python3
"""
attractor_2scroll_slider_v2.py — Wang 2-Scroll: Schieberegler a, d, b, c.

Schieberegler: a, d, b, c  (IC fix: [4.84, 0.25, -5.44])
H1-Fläche + Schnittkurve (H1∩H2) live.
Gleichgewichte reagieren auf alle 4 Parameter.
"""
...
# ── Startwerte ──────────────────────────────────────────────────────────────
A0, B0, C0, D0 = 3.5, 9.0, 5.0, 0.06
IC = np.array([4.84, 0.25, -5.44])
...
def h2(x, y, z, a, d):
    arg = np.maximum(np.abs(z + y + d), 1e-12)
...
params = [
    ('a', 0.1, 5.0,  A0, 'a'),
    ('d', -0.5, 0.5, D0, 'd'),
    ('b', 1.0, 20.0, B0, 'b  (y-Diss)'),
    ('c', 0.5, 12.0, C0, 'c  (z-Diss)'),
]
sliders = {}
for i, (name, vmin, vmax, v0, label) in enumerate(params):
    sax = fig.add_axes([0.12, 0.22 - i*0.04, 0.75, 0.025], facecolor='#1A1A1A')
...
DEFAULT_AZIM, DEFAULT_ELEV = -153, 6
_view = {'azim': DEFAULT_AZIM, 'elev': DEFAULT_ELEV, 'dist': 10, 'init': False}
...
def update(_=None):
    ...
    for i_l, loop in enumerate(loops):
        ax.plot(loop[:,0], loop[:,1], loop[:,2],
                color=colors[i_l % len(colors)], linewidth=2.2, alpha=0.95)

    eq_pts = equilibria(a, b, c, d)
    for i_e, (lbl, pt) in enumerate(eq_pts):
        col = EQ_COLORS[i_e % len(EQ_COLORS)]
    ...
    ax.set_title(
        f'Wang 2-Scroll  a={a:.3f} b={b:.2f} c={c:.2f} d={d:.3f}'
        f'  k1={k1:.3f} k2={k2:.3f}  Loops={len(loops)}',
