#!/usr/bin/env python3
"""Schritt 2: 3D-SSM (PC1+PC2+PC3) -- alle Sub-Harmonischen markiert.

Erweiterung von 13_cli_lambda2_visualize.py:
- ssm_dim default = 3 (PC3 dazu)
- Top-Plots: PC3 als zusaetzliche Linie
- Eigenwert-Plots: ALLE oszillatorischen Paare beschriftet (Master + Subs)

Top    -- linear-time SSM (PC1, PC2, PC3 vs day)
Mitte  -- log10-time SSM   (PC1, PC2, PC3 vs log10(day))
Unten  -- Eigenwerte mit Master und Sub-Harmonischen
"""
...
def build_one_analysis(filename, M, years, start_idx, ssm_dim, poly, time_mode,
    ...
    if use_log_uniform:
        ...
        payload = build_n_log_uniform_context(
            ...
            target_mean_w_days=target_mean,
            time_mode=time_mode,
        )
    else:
    ...
    pc1 = pc[:, 0]
    pc2 = pc[:, 1]
    pc3 = pc[:, 2] if pc.shape[1] >= 3 else None
    ...
    T_master = None
    lam_fit = None
    osc_pairs = []  # list of (lam, T, lam_factor)
    if eigs is not None and len(eigs) >= 2:
        pos_pairs = sorted(
            [e for e in eigs if e.imag > 1e-12],
            key=lambda e: abs(e.imag),
        )
        if pos_pairs:
            lam_master = pos_pairs[0]
            T_master = 2.0 * np.pi / abs(lam_master.imag)
            if time_mode == 'log':
                lam_fit = 10.0 ** T_master
            for e in pos_pairs:
                T_e = 2.0 * np.pi / abs(e.imag)
                lam_e = 10.0 ** T_e if time_mode == 'log' else None
                osc_pairs.append({'lam': e, 'T': T_e, 'lam_factor': lam_e})
    fit_err = float(res.get('fit_err', float('nan')))
    var = ctx.get('var')
    cum_var_n = float(np.sum(var[:ssm_dim]) * 100.0) if var is not None else float('nan')
    return {
        'time_mode': time_mode,
        'days': days,
...
def plot_eigenvalues(ax, ana, title, legend_loc='upper right',
    ...
    osc_pairs = ana.get('osc_pairs', [])
    sub_colors = ['#FFEB3B', '#4DABF7', '#69DB7C', '#FF6B6B', '#B197FC']
    for i, p in enumerate(osc_pairs):
        e = p['lam']
        T_e = p['T']
        col = sub_colors[i % len(sub_colors)]
        if i == 0:
            tag = r'master'
        else:
            tag = r'sub' + str(i)
        if ana['time_mode'] == 'log':
            lab = tag + r': $T = ' + f'{T_e:.2f}' + r',\ \lambda = ' + f'{p["lam_factor"]:.2f}' + r'$'
        else:
            T_y = T_e / 365.25
            lab = tag + r': $T = ' + f'{T_e:.0f}\,\mathrm{{d}} = ' + f'{T_y:.2f}\,\mathrm{{y}}' + r'$'
        ax.scatter([e.real, e.real], [e.imag, -e.imag], s=80, facecolors='none',
                   edgecolors=col, linewidth=2.2, zorder=11, label=lab)
    if ana['lam_master'] is not None:
        alpha = ana['lam_master'].real
        omega = ana['lam_master'].imag
        T = ana['T_master']
        fit_err = ana.get('fit_err', float('nan'))
        cum_var = ana.get('cum_var_n', float('nan'))
        ssm_dim = ana.get('ssm_dim', 2)
        Q = abs(omega) / (2.0 * abs(alpha)) if abs(alpha) > 1e-30 else float('inf')
        if ana['time_mode'] == 'log':
            text_phys = (r'$\alpha = ' + f'{alpha:+.3e}' + r'$' + '\n' +
                         r'$\omega = ' + f'{omega:+.3e}' + r'$' + '\n' +
                         r'$T_{\mathrm{master}} = ' + f'{T:.3f}' + r'$' + '\n' +
                         r'$\lambda = 10^T = ' + f'{ana["lambda_fit"]:.3f}' + r'$')
        else:
            T_y = T / 365.25
            text_phys = (r'$\alpha = ' + f'{alpha:+.3e}' + r'$' + '\n' +
                         r'$\omega = ' + f'{omega:+.3e}' + r'$' + '\n' +
                         r'$T_{\mathrm{master}} = ' + f'{T:.0f}\,\mathrm{{d}} = ' + f'{T_y:.2f}\,\mathrm{{y}}' + r'$')
        text_err = (r'decoder\_err $= ' + f'{fit_err:.3f}' + r'$, ' +
                    r'cum\_var(' + str(ssm_dim) + r') $= ' + f'{cum_var:.1f}\,\%' + r'$, ' +
                    r'Q $= ' + f'{Q:.2f}' + r'$')
    ...
    leg = ax.legend(loc=legend_loc, fontsize=9, facecolor='#1A1A1A',
                    edgecolor='#808080', labelcolor='#E0E0E0', framealpha=0.85)
    leg.set_zorder(50)
    if len(re) > 0:
...
def plot_time_series(ax, ana, axis_clock):
    ...
    if ana.get('pc3') is not None:
        ax.plot(x, ana['pc3'], color=COLOR_PC3, lw=0.7, alpha=0.7, label='PC3')
...
def main():
    ...
    ap.add_argument('--ssm_dim', type=int, default=5)
    ...
    ap.add_argument('--log_uniform_mean', type=float, nargs='+', default=None,
                    help='If set, SSM B uses log-uniform window with mean=X days '
                         '(scale-invariant filter in log-clock). Multiple values '
                         'create an ensemble (n(t) averaged over given means). '
                         'SSM A unaffected unless --log_uniform_both.')
    ap.add_argument('--log_uniform_both', action='store_true',
                    help='Apply log-uniform-window n(t) to BOTH SSMs (mode b cross-check). '
                         'Requires --log_uniform_mean.')
    ap.add_argument('--no-show', action='store_true')
    ap.add_argument('--no-marker', action='store_true',
                    help='Skip peak detection and percent-distance markers on top plots')
    ...
    if not args.no_marker:
        ...
        _idx_lin = np.array([], dtype=int)
        halv_ext = HALVING_DAYS + [HALVING_DAYS[-1] + 3.9 * 365.25]
    ...
    if not args.no_marker:
        ...
        _idx_pc2_lin = np.array([], dtype=int)
    ...
    if args.save is None:
        ...
        args.save = os.path.join(
            HERE,
            f'13_2_lambda2_visualize_dim{args.ssm_dim}_poly{args.poly}_{wtag}.png',
        )
