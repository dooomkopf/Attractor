#!/usr/bin/env python3
"""03b (n_ens): Phase-Lock test aller oszillatorischen Paare in SSM B fuer n_ens.

Identisch zu analyze_residuals/03b_cli_phase_all.py, aber Eingangssignal ist die
Ensemble-LPPL-Exponentenkurve n(t). Default: single 180d-Fenster (gibt sauberen
lambda=2-Master).

CLI:
  ./03b_cli_phase_all.py --ssm_dim 9 --poly 2 --windows 180
"""
...
def main():
    ap = argparse.ArgumentParser(
        description='Phase-Lock test for ALL SSM B oscillatory pairs (n_ens)')
    ...
    ap.add_argument('--windows', type=int, nargs='+', default=[180],
                    help='Window sizes for n_ens; default = [180] (single window)')
    ...
    payload = build_n_ens_context(args.filename, args.M, args.years, args.start_idx,
                                   time_mode='log', window_sizes=args.windows)
    ctx = payload['ctx']
    ...
    for i in range(1, n_pairs):
        ...
        for k in range(2, 9):
            ...
            if not is_harm:
                ...
                if R_k > 0.5:
                    verdict = '*** HARMONIC, SLAVED'
                elif R_k > 0.3:
                    verdict = '** HARMONIC, slaved (moderate)'
                elif R_k > 0.15:
                    verdict = '** HARMONIC, weak lock'
                else:
                    verdict = '*** HARMONIC, INDEPENDENT (DSI)'
                if amp_corr > 0.2:
                    verdict += ' +amp-coupled'
                elif amp_corr < -0.2:
                    verdict += ' +amp-anti'
    ...
    print('FREQUENCY RATIOS  omega_i / omega_j  (look for integer ratios = harmonic relationship)')
    print(header)
    for i in range(n_pairs):
        row = f'  [{i}]  '
        for j in range(n_pairs):
            ratio = pairs[i]['mu'].imag / pairs[j]['mu'].imag
            row += f' {ratio:6.3f} '
        print(row)
    print()
    ...
    # Per-mode classification labels
    ...
    # ---- Harmonic classification with leeway ----
        ...
        for k in range(2, 9):
            ...
            if not is_harm:
                ...
                if amp_corr > 0.2:
                    ...
                    verdict += '  +amp-anti'
            print(f'  [{i}]  {k}  {ratio:5.2f}  {dev*100:4.1f}%  '
                  f'{R_k:5.3f}  {amp_corr:+5.3f}    {verdict}')
    print()

    # Polar: master (mode 0) vs FIRST REAL HARMONIC = lowest k from classifications
    # that is is_harm=True (ratio dev < 15% from integer).
    # Auto-adapts to ssm_dim — if higher dim reveals lower-k harmonic, polar follows.
    om0 = pairs[0]['mu'].imag
    ...
    real_harm = sorted([c for c in classifications if c['is_harm']],
                       key=lambda c: c['k'])
    if real_harm:
        if hasattr(args, 'harmonic_k') and args.harmonic_k is not None:
    ...
    if harm_idx is not None:
        ...
        print(f'\nPOLAR PLOT focus: mode {harm_idx} as {harm_k}. harmonic of master')
        print(f'  ratio omega_{harm_idx}/omega_0 = {actual_ratio:.3f}  '
              f'(integer {harm_k}, dev {abs(actual_ratio-harm_k)/harm_k:.1%})')
        print(f'  Wang-form psi-lock R = {R_h:.4f}')
    else:
        ax_pol.text(0, 0, 'no integer harmonic\ndetected',
                    ha='center', va='center', color='#E0E0E0', fontsize=10)
        ax_pol.set_title('No integer harmonic candidate', fontsize=10)

    win_str = '-'.join(str(w) for w in args.windows)

    # Theoretical harmonic table (matplotlib Table)
    lam0_obs = 10.0 ** pairs[0]['T']
    ks = list(range(1, 9))
    th_lam = [lam0_obs ** (1.0 / k) for k in ks]
    obs_lam = [10.0 ** p['T'] for p in pairs]
    obs_k = []
    for ip, p in enumerate(pairs):
        if ip == 0:
            obs_k.append(1)
        else:
            obs_k.append(round(p['mu'].imag / pairs[0]['mu'].imag))
    # Vertical table: rows = k, cols = theor / obs
    cell_text = [['k', r'theor. $\lambda$', r'obs. $\lambda$', 'mode']]
    obs_by_k = {k_obs[0]: (10**pairs[i]['T'], i) for i, k_obs in [(idx, [obs_k[idx]]) for idx in range(len(obs_k))]}
    for k in ks:
        if k in obs_by_k:
            obs_lam_v, mode_idx = obs_by_k[k]
            cell_text.append([str(k), f'{lam0_obs**(1.0/k):.3f}',
                              f'{obs_lam_v:.3f}', f'{mode_idx}'])
        else:
            cell_text.append([str(k), f'{lam0_obs**(1.0/k):.3f}', '–', '–'])
    ax_tbl.axis('off')
    ...
    for (r, c), cell in tbl.get_celld().items():
        ...
        if r == 0:
            ...
            cell.set_facecolor('#2D4A2D')  # green tint = matched k
    ax_tbl.set_title(rf'Theoretical vs observed  ($\lambda_0 = {lam0_obs:.3f}$)',
                     fontsize=11, color='#E0E0E0', pad=4)

    with plt.rc_context({'text.usetex': False}):
        plt.suptitle(f'BTC Ensemble n: phase-lock of all SSM B (log10-clock) modes  '
                     f'[ssm_dim={args.ssm_dim}, poly={args.poly_degree}, windows={win_str}]',
    ...
    if args.save is None:
        args.save = os.path.join(
            HERE,
            f'03b_phase_all_SSM_B_dim{args.ssm_dim}_poly{args.poly_degree}_w{win_str}.png',
        )
