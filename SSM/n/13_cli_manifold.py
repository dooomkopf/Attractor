#!/usr/bin/env python3
"""13: Manifold (n) — polynomial decoder W(PC1,PC2)->PC3.. fit + 3D manifold plot.

Signal: n_180(t) = (log p(t+90) - log p(t-90)) / (log(t+90) - log(t-90)).
"""
...
import numpy as np
from SSM_n_data import load_data
from SSM_n_embedding import build_embedding, pca, smooth_pcs
from SSM_n_geometry import fit_geometry, scan_orders
from SSM_n_phase import compute_phase_full
from SSM_n_plots_manifold import plot_manifold
...
def main():
    ...
    ap.add_argument('--start-idx', type=int, default=1229,
                    help='Erster Tag fuer Embedding (default 1229 = erster '
                         'valider n(t)-Tag bei half=90 und Datenstart 394).')
    ...
    ap.add_argument('--fracdiff', nargs='?', const=0.2, default=None, type=float,
                    metavar='d',
                    help='Periodic signal filter mit Parameter d in (0,1). '
                         ...
                         'Beispiel: --fracdiff 0.2. Weggelassen: aus.')
    ...
    if args.time_mode == 'log':
        ...
        dates_ord = np.array([d.toordinal() for d in data['dates_emb']], dtype=float)
        ord_back = np.interp(tau_grid, tau_raw, dates_ord)
        dates_back = np.array([datetime.fromordinal(int(round(o))) for o in ord_back])
        data = dict(data)
        data['signal'] = sig_tau
        data['days_emb'] = days_back
        data['dates_emb'] = dates_back
    ...
    if args.scan_orders:
        ...
        print("=" * 72)
        print("SSM/n MANIFOLD ORDER SCAN")
        print("=" * 72)
    ...
    if args.show:
        ...
        plot_manifold(pca_res, pc_s, fit_for_plot,
                      ...
                      signal_label=data['signal_label'], fig_num=3)
        plt.show()
