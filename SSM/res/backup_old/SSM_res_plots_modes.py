"""SSM/res/plots_modes.py — PCA-Mode-Plot + Variance-Spektrum.

Fig 1: plot_modes      — V[k, :] über die Lag-Achse
Fig 2: plot_variance   — Scree (log) + kumulative Varianz
"""
...
def _flip_modes_for_visualization(Vt, K_show):
    ...
    SVD-Vorzeichen ist beliebig, hat keinen Effekt auf den Fit, ist aber für
    die Optik konsistenter.
    """
    Vt_out = Vt.copy()
...
def plot_modes(pca_res, M, TAU, K_show=6, mode_label='log res', fig_num=1):
    """Fig 1 — PCA modes V[k, :] über die Lag-Achse j·TAU in Tagen.

    K_show ist hart auf max. 6 begrenzt (Default 6) — User-Vorgabe:
    "bis Mode 6, riecht".
    """
    K_show = min(int(K_show), 6, pca_res.Vt.shape[0])
    Vt_flipped = _flip_modes_for_visualization(pca_res.Vt, K_show)

    n_cols = 2
    n_rows = (K_show + 1) // 2
    fig = plt.figure(figsize=(11, 1.6 * n_rows + 1.4), num=fig_num)
    ...
    with plt.rc_context({'text.usetex': False}):
        fig.suptitle('Residual SSM — PCA modes',
...
def plot_variance(pca_res, K_show, signal_label='log res', fig_num=2):
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 4.2), num=fig_num)
    ...
    with plt.rc_context({'text.usetex': False}):
        fig.suptitle(f'Residual SSM — variance spectrum  (signal: {signal_label})',
                     color='#CCCCCC', fontsize=13, y=0.985,
                     fontname='Comfortaa', fontweight='bold')
