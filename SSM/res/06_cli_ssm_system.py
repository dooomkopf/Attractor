#!/usr/bin/env python3
"""CLI for LPPL/SSM system precheck on LPPL simulation dynamics."""
...
from lppl_system import (
    DEFAULT_PARAMS, build_params, find_equilibria, lppl_jacobian,
...
def main():
    ap = argparse.ArgumentParser(description='LPPL model precheck (SSM-style system screen)')
    ...
    print("=" * 84)
    mode_tag = "QUASI-2D (--wang-off: Z_A=0, Z_B=0, Z_MIX=8e-5)" if args.wang_off else "FULL 3D (Wang coupling active)"
    print(f"LPPL MODEL PRECHECK — {mode_tag}")
    print("=" * 84)
    print("SYSTEM")
    ...
    print(f"  SSMLEARN        : available   [from LPPL simulation trajectory]")
